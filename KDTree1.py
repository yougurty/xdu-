...
class kdTree:
    ...
    def getNodeLeftRigthData(self, all_data):
        ...
        for i in range(data_count):
            ...
            elif now_data[self.split].all() == self.range.all() and self.nodedata == None:
...
data = LoadTxtMethod2('NE.txt')  # 调用上面数据处理程序
# test_array1 = 10.0*np.random.random([10000,2])   ### 随机生成n个2维0-10以内的数据点
# print(type(test_array1))

my_kd_tree = kdTree(None)                    ### kd树实例化
my_kd_tree.createNextNode(data)              ### 生成kd树
...
min_dist = my_kd_tree.getNestNode(find_data, min_dist, min_data)        ### 利用kd树回溯查找
min_dist = my_kd_tree.getNestDistByEx(data, find_data, min_dist, min_data)    ### 穷举法查找
